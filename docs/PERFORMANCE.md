...
Gate 8B repeated the same three-run audit without changing budgets. Resource maxima were unchanged from Gate 8A: 9 JavaScript requests / 145,010 encoded bytes, 3 CSS requests / 13,909 bytes, 3 local-font requests / 50,016 bytes, and 30 total requests. Median lab LCP ranged from 104–164ms and CLS from 0.00020–0.03198; the timing spread is treated as local lab noise because deterministic resource counts and bytes did not regress.

Gate 9A retained every budget while adding only server-rendered/static acquisition surfaces. The three-run audit measured maxima of 9 JavaScript requests / 145,346 encoded bytes, 3 CSS requests / 13,909 bytes, 3 local-font requests / 50,016 bytes, and 29 total requests. Median lab LCP ranged from 72–128ms and CLS from 0.00020–0.03198. No audited page contacted a third-party host, emitted a console error, or overflowed.

Gate 10A's production CSP adds a response header and no client dependency, request, or asset. A nonce-based alternative was rejected for this gate because Next.js 16 requires dynamic rendering for nonce-protected pages, removing static optimization and normal CDN caching. The established 9 JavaScript / 150,000 bytes, 3 CSS / 14,500 bytes, 3 font / 52,000 bytes, and 31 total-request ceilings remain unchanged and are rechecked before release.
