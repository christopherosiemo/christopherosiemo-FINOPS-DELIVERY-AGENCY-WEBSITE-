...
`pnpm build:cf` is the authoritative Cloudflare build. It runs `vinext build` and then inspects the generated Worker artifact for the exact approved CSP; a missing or changed policy fails before upload. `pnpm test:production-cf-headers` builds by that same path, starts `dist/server/wrangler.json` under the local Workers runtime, verifies the root, conversion, security, privacy, and hard-404 response headers, and dry-runs the named production deployment configuration. `pnpm deploy:cf --env <environment>` first runs that guarded build, requires `staging` or `production`, merges the selected named environment onto the qualified artifact, and invokes Wrangler without rebuilding. This prevents the redirected vinext configuration from silently falling back to the top-level Worker name.
...
Gate 10A adds a production-readiness harness that runs an explicit production build with the Cloudflare Turnstile test site key. It verifies the static CSP and retained headers, route/navigation behavior, focus and overflow, actual Turnstile script instantiation, zero collected CSP violations, and no form POST. Component tests protect route-free production configuration, dedicated hostname/site-key separation, required bindings and secrets, fail-closed dependency checks, and production test-mode/query-scenario rejection. CI runs this harness in addition to the established regression suites.
