...
The accurate customer-facing privacy disclosure is: “We will use the information you submit to respond to this enquiry and assess whether a HKGpipi engagement is appropriate.” It links to the approved `/privacy` policy. No mandatory marketing consent or newsletter opt-in is present. `/start`, `/privacy`, and `/contact` remain `noindex, nofollow` under the pre-acquisition policy.

## Submission architecture

The form posts to a Next.js Server Action and uses React action state only for pending-state, Turnstile, and focus enhancement. Static page context is server-rendered. With JavaScript disabled, the protected form stays disabled and presents the authorised `enquiries@hkgpipi.com` mailto fallback; no submitted value is placed in a URL. The Server Action is the untrusted boundary: it reads an explicit allow-list, validates and normalises fields, checks origin and honeypot, applies the durable rate limit, validates Turnstile, creates a request ID, and calls the delivery interface. Delivery credentials and the Turnstile secret never enter client JavaScript.
...
The off-screen honeypot remains outside the keyboard order. The under-one-second timestamp heuristic no longer rejects a submission: Managed Turnstile and durable rate limiting provide stronger controls without penalising autofill, assistive technology, or very fast legitimate visitors. Turnstile Siteverify runs only after field validation and rate limiting. A SQLite-backed Durable Object allows five valid-shaped attempts per HMAC-pseudonymised network key per 15 minutes; missing IP enters a shared fallback bucket rather than bypassing protection.
...
Gate 7B reaches at most 77% after staging deployment and one controlled send. The remaining 78% requirement is:

- human confirmation that the controlled message arrived at the authorised destination with matching subject/reference, correct body and Reply-To, and no IP/token/secret leakage.
