...
Gate 10B.2 removes the release-pipeline dependency between compiled CSP and the runtime-only `APP_ENVIRONMENT` variable. `pnpm build:cf` now performs the vinext build and an exact generated-artifact CSP assertion. `pnpm test:production-cf-headers` starts that output with Wrangler and verifies the complete header boundary on representative routes and the hard 404. `pnpm deploy:cf --env <environment>` runs the guarded build and deploys the same output with `--skip-build`; do not invoke `vinext-cloudflare deploy` directly as an alternative release path.
